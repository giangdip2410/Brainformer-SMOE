...
def _get_optimizer(model, optim, lr: float, momentum: float, grad_clip: float):
    if optim == "sgd":
        return torch.optim.SGD(
            _get_grad_requiring_params(model), lr=lr, momentum=momentum
        )
    elif optim == "adagrad":
        return AdagradWithGradClip(
            _get_grad_requiring_params(model), lr=lr, grad_clip=grad_clip
        )
    else:
        raise RuntimeError("wrong type of optimizer " "- must be 'sgd' or 'adagrad")
...
def get_optimizer_and_scheduler(model, optim_params):
    ...
    scheduler = _get_scheduler(optimizer=optimizer, lr_warmup=optim_params["lr_warmup"])
...
def save_checkpoint(checkpoint_path, iter_no, model, optimizer, scheduler, logger):
    if checkpoint_path:
        checkpoint_state = {
