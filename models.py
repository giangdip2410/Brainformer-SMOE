...
class CustomizedMoEPositionwiseFF(FMoETransformerMLP):
    def __init__(
        self, d_model, d_inner, dropout, pre_lnorm=False, moe_num_expert=16, moe_top_k=2
    ):
...
class TransformerSeqLayer(nn.Module):
    def __init__(self, hidden_size, arch, inner_hidden_size, dropout, **kargs):
        nn.Module.__init__(self)

        self.attn = MultiHeadSeqAttention(
            hidden_size=hidden_size, dropout=dropout, **kargs
        )
        ...
        self.g = CustomizedMoEPositionwiseFF(
            d_model=hidden_size, d_inner=inner_hidden_size, dropout=dropout
        )
...
class TransformerSeq(nn.Module):
    def __init__(
        ...
        **kargs
        ...
        self.arch_list = arch.split(".")
        self.layers = nn.ModuleList()
        self.nb_layers = len(self.arch_list)
        self.attn_layer_count = len(self.arch_list)  # arch.count('s')
        self.layers.extend(
            TransformerSeqLayer(
                hidden_size=hidden_size,
                arch=self.arch_list[i],
                nb_heads=nb_heads,
                inner_hidden_size=inner_hidden_size,
                dropout=dropout,
                attn_span=attn_span,
